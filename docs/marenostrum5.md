...
- `VEC_INF_STORAGE_USER`: only when the storage path owner differs from your login user.
- `REMOTE_ACCOUNT`: defaults to `bsc70` in the wizard because that is the current team example in this repo.
- `VEC_INF_VLLM_IMAGE_PATH`: defaults to `/gpfs/scratch/bsc70/hpai/storage/users/<storage-user>/singularity/vllm_<storage-user>.sif`.
...
- `launch_and_tunnel.sh` exports `VEC_INF_STORAGE_USER` so model entries can still refer to user-owned storage paths when needed.
